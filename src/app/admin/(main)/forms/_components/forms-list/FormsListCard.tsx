...
import { useForms } from '@/hooks/api/admin/use-forms';

export default function FormsListCard() {
    ...
    <Card variant="primary" className="w-full overflow-hidden px-4 py-3">
      ...
      {data && data.results.length === 0 && (
        <div className="flex h-[148px] items-center justify-center px-4">
          <p className="text-paragraph-sm text-neutral-700/68">No forms yet.</p>
        </div>
      )}

      {data && data.results.length > 0 && <FormsList items={data.results} />}
    </Card>
  );
}
