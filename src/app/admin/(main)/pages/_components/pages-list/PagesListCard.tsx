...
export default function PagesListCard() {
    ...
    <Card variant="primary" className="w-full overflow-hidden px-4 py-3">
      ...
      {data && data.pages.length === 0 && (
        <div className="flex h-[148px] items-center justify-center px-4">
          <p className="text-paragraph-sm text-neutral-700/68">No pages yet.</p>
        </div>
      )}

      {data && data.pages.length > 0 && <PagesList items={data.pages} />}
    </Card>
  );
}
