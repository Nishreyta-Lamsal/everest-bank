...
import RecentFilesToolbar, { type ViewMode } from './RecentFilesToolbar';
import RecentFilesGrid from './RecentFilesGrid';
import RecentFilesTable from './RecentFilesTable';
import RecentFilesPagination from './RecentFilesPagination';
...
export default function RecentFilesCard() {
  ...
  // Browsing and searching both page by cursor, but a cursor from one endpoint
  // is meaningless to the other, so each keeps its own trail.
  const listPagination = useCursorPagination();
  const searchPagination = useCursorPagination();
  // Face search returns a fixed set rather than a page, so it replaces the
  // list entirely until cleared.
  ...
  const listQuery = useMediaList(
    {
      ordering,
      ...(cursor ? { cursor } : {}),
      ...(folderId ? { folder: Number(folderId) } : {}),
    },
    !isSearching && !isFaceSearching,
  );

  // A typed query goes through the plain title search, which takes the same
  // `ordering` and cursor as the list.
  ...
  /** Any filter change invalidates both paging trails. */
    ...
    <>
      <div className="flex w-full flex-col rounded-[8px] border border-white bg-white/90 shadow-[0px_1px_2px_0px_rgba(15,23,42,0.04)]">
        <RecentFilesToolbar
          ...
          folderId={folderId}
          onFolderChange={resetPaging(setFolderId)}
        ...
        <RecentFilesPagination
          ...
          // `has_more` is the endpoint's own answer; fall back to whether it
          // handed us a cursor for the endpoints that omit the flag.
          hasNext={
            !isFaceSearching &&
